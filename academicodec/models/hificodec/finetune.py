...
def train(rank, a, h):
    ...
    for epoch in range(max(0, last_epoch), a.training_epochs):
        ...
        if epoch < h.emo_warmup_epochs:
            ...
            for param in encoder.parameters():
                param.requires_grad = False
            if not a.freeze_quantizer:
                for param in quantizer.parameters():
                    param.requires_grad = False
            if not a.freeze_generator:
                for param in generator.parameters():
                    param.requires_grad = False
        ...
        for i, batch in enumerate(train_loader):
            ...
            if hasattr(loss_disc_all, 'backward'):
            ...
            if rank == 0:
                ...
                if steps % a.stdout_interval == 0:
                    ...
                    print(
                        'Steps : {:d}, Gen Loss Total : {:4.3f}, Loss Q : {:4.3f}, Mel-Spec. Error : {:4.3f}'.
                ...
                if steps % a.summary_interval == 0 and sw is not None:
                    sw.add_scalar("training/gen_loss_total", loss_gen_all, steps)
                    sw.add_scalar("training/mel_spec_error", mel_error, steps)
                # Validation
                if steps % a.validation_interval == 0 and steps != 0 and validation_loader is not None:
                    generator.eval()
                    encoder.eval()
                    quantizer.eval()
